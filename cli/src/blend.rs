...
//!
//! The output is a library-mode `.blend` (no scene graph) containing a single
...
///
/// Produces a library-mode `.blend` containing a single OB_MESH object named
/// `name`.  The mesh indices are expected to be triangles (3 indices per face).
///
/// Attributes written:
