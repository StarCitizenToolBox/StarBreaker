...
//! non-transparent pixel in the rendered output is multiplied component-wise by the
//! override colour.  Because the internal pixmap uses premultiplied RGBA, this
//! multiplication is applied directly to the raw bytes, which preserves premultiplied
//! correctness for the RGB channels and scales the overall opacity by `fill[3]`.
...
/// non-transparent pixel is multiplied component-wise by the override colour after
/// rendering.
///
/// Returns `None` when:
...
pub fn rasterize_svg(
    ...
    // Apply fill_override: multiply every non-transparent pixel by the override colour.
    // The pixmap stores premultiplied RGBA, so direct component-wise multiplication of
    // the raw bytes preserves premultiplied correctness.
    if let Some(fill) = fill_override {
        for chunk in bytes.chunks_exact_mut(4) {
            if chunk[3] > 0 {
                chunk[0] = (chunk[0] as f32 * fill[0]).clamp(0.0, 255.0) as u8;
                chunk[1] = (chunk[1] as f32 * fill[1]).clamp(0.0, 255.0) as u8;
                chunk[2] = (chunk[2] as f32 * fill[2]).clamp(0.0, 255.0) as u8;
                chunk[3] = (chunk[3] as f32 * fill[3]).clamp(0.0, 255.0) as u8;
            }
        }
    }
